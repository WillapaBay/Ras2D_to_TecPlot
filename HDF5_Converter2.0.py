...
class Ras2D:
    def __init__(self, projectFilename, fort63Filename, fort64Filename, tinFilename, outputPath):
        ...
        waterSurfaceElevations = self.data["WaterSurfaceElevations"]
    ...
    def plotVelocities(self, scale=1.0, fileExtension="png"):
        ...
        for timeStamp in self.data["Times"]:
            ...
            U = np.array(self.data["U"][timeIndex])
            V = np.array(self.data["V"][timeIndex])
    ...
    def plotDepths(self, fileExtension="png", xmin=0.0, xmax=10000.0, dx=4.0,
        ...
        for timeStamp in self.data["Times"]:
            ...
            timeIndex = self.data["Times"].index(timeStamp)
            Z = np.array(self.data["Depths"][timeIndex])
...
if __name__ == "__main__":
    ...
    ras2d = Ras2D(projectFilename, fort63Filename, fort64Filename, tinFilename, outputPath)

    meanDepth = np.mean(np.transpose(ras2d.data["Depths"])[0])
    print(meanDepth)

    ras2d.writeToTecPlot()
    ras2d.plotVelocities(scale=vectorScale)
    # ras2d.plotDepths(fileExtension="png", xmin=404000.0, xmax=413000.0, dx=100.0, ymin=1800000.0,
